...
import type { CartItem } from '@/types'; // Assuming CartItem is defined in types
...
export function OrderViaWhatsAppButton({ 
  productName, 
  cartItems,
  className, 
  buttonText,
  ...
  let message: string;
  const defaultGreeting = "Hello Matesh Technologies,";

  if (cartItems && cartItems.length > 0) {
    const itemsList = cartItems.map(item => `${item.name} (Qty: ${item.quantity})`).join("\n - ");
    message = `${defaultGreeting} I would like to order the following items:\n - ${itemsList}\n\nPlease let me know how to proceed.`;
  } else if (productName) {
    message = `${defaultGreeting} I would like to order the product: ${productName}. Please let me know how to proceed.`;
  } else {
    message = `${defaultGreeting} I have an inquiry.`; // Fallback message
  }
  ...
  return (
    ...
    >
      ...
      >
        <WhatsAppIcon />
        <span className="ml-2 text-sm font-medium">{buttonText || "Order via WhatsApp"}</span>
      </Link>
    </Button>
  );
}
